...
\begin{wrapfigure}{r}{0.36\textwidth}
  ...
  \setlength{\fboxsep}{0pt}
  \setlength{\fboxrule}{0.6pt}
  \fbox{\includegraphics[width=\linewidth]{img/5.5_reprtoire_A_infos.jpg}}
  \caption{Interface du Géomètre A (port 5050) -- Fiche détaillée et édition des champs.}
  \label{fig:geom_a_infos}
\end{wrapfigure}

L'interface s'articule autour de deux vues principales. La première (Figure~\ref{fig:geom_a_liste}) présente la liste des dossiers extraits sous forme de tableau, permettant à l'opérateur de parcourir l'ensemble des affaires répertoriées.

Lorsqu'un dossier est sélectionné, l'interface bascule sur la fiche détaillée (Figure~\ref{fig:geom_a_infos}), qui regroupe l'ensemble des champs d'indexation : commune, année, référence du dossier, anciens et nouveaux propriétaires. L'opérateur peut y consulter le scan de la page originale, relancer l'OCR sur une zone spécifique ou corriger directement les valeurs extraites.

À la validation, les corrections sont fusionnées dans le fichier Excel principal nommé {\small\texttt{Repertoire\_Archives\_geometre\_A.xlsx}}, en utilisant les clés d'indexation (Commune, Année, N° Dossier) pour éviter tout doublon. Le fichier est également synchronisé sur le serveur partagé du cabinet. (Les captures grand format sont consultables en Annexe G.)

Le traitement repose sur une logique de découpage par colonnes fixes. Les lignes de séparation imprimées en rouge permettent d'isoler les noms des anciens et nouveaux propriétaires. En cas de doute sur une lettre ou un nom propre, l'interface permet à l'opérateur de visualiser immédiatement le bloc d'image découpé et de rectifier la saisie en quelques secondes.
...
Cette étape de numérisation préalable des registres permet de construire la base d'indexation indispensable à la recherche d'antériorités. Dans les deux cas, les données brutes issues de l'OCR ou du modèle VLM ne peuvent pas être exploitées directement : les noms propres, et en particulier les noms de clients ou des voisins, concentrent l'essentiel des erreurs. La section suivante détaille comment ces erreurs sont détectées et corrigées avant l'enregistrement définitif.


\subsection{Résolution et correction des noms de communes}
\label{subsec:appariement_communes}

Lors de l'extraction des données par les modèles de lecture, le texte transcrit pour les noms de communes contient souvent des erreurs de nature variée : un caractère mal lu, une majuscule différente, une abréviation ou autre. Par exemple, le moteur peut lire \texttt{St-Pierreville} au lieu de \texttt{Saint-Pierreville}, ou \texttt{Chomerac} au lieu de \texttt{Chomérac}. Ces erreurs empêchent une correspondance parfaite avec la liste de référence des communes, ou à l'inverse peuvent faire correspondre une détection à la mauvaise commune, alors que cette dernière est en realité pertinente. Il est donc nécessaire de mettre en place un système de correction automatique de ces erreurs.
...
où $a$ est la chaîne extraite, $b$ le nom de commune de référence, $n = |a|$ et $m = |b|$.Le score exprimé sur 100 par la bibliothèque \texttt{rapidfuzz} correspond à un ratio de similarité dérivé de cette distance : $\text{ratio}(a, b) = 100 \times \left(1 - \frac{d(a,b)}{\max(|a|,|b|)}\right)$. Ce score est calculé en quelques microsecondes grâce aux optimisations SIMD de RapidFuzz, ce qui rend la recherche instantanée même sur la liste complète des communes françaises, réduite aux communes de l'Ardèche ici.
...
\begin{itemize}
  ...
  \item Correspondance sur les mots : les mots de chaque chaîne sont triés avant la comparaison. Ce traitement gère les inversions d'ordre, comme \texttt{Bourg-Saint-Andéol} comparé à \texttt{Saint-Andéol-de-Berg}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{img/zoom_commune.jpg}
  \caption[Résultat de l'appariement pour le champ Commune]{Résultat de l'appariement : le champ Commune affiche la valeur \texttt{Joyeuse} avec son code INSEE et un score de confiance de 100\,\%.}
...
Pour accélérer le calcul lors du traitement de gros volumes de documents, la recherche par correspondance s'appuie sur un index pré-calculé chargé en mémoire au démarrage de l'application. Cet index regroupe les noms de communes par département et par préfixe alphabétique, ce qui réduit le nombre de comparaisons nécessaires et permet d'obtenir une réponse en moins d'une seconde par mot.

Pour chaque proposition de correction générée par la comparaison de Levenshtein, l'interface conserve le détail de la chaîne initiale extraite par l'OCR, la méthode de calcul ayant obtenu le meilleur score (directe, par tokens ou partielle), ainsi que les deux communes candidates arrivées en deuxième et troisième positions. Ces informations permettent à l'opérateur de comprendre rapidement l'origine d'une alerte lors d'un doute sur une commune limitrophe, et à l'avenir améliorer le système de reconnaissance.

\subsection{Bilan de la phase de fiabilisation}

La combinaison du nettoyage automatique et du contrôle humain via les interfaces web permet d'obtenir des données fiables et directement exploitables. Les deux extracteurs (Géomètre A et Géomètre B) adoptent des stratégies très différentes dictées par la nature des registres, des colonnes fixes et marqueurs couleur pour l'un, découpe ligne par ligne et modèle VLM pour l'autre, mais débouchent tous deux sur le même format, prêt à l'indexation. Sur le registre du Géomètre A, l'extraction automatique a produit 97 dossiers couvrant la période 1970--1991, dont 27 signalés avec au moins un champ incomplet (absence d'année ou de colonne \textit{Nouveaux}). Ces avertissements ne bloquent pas le processus : ils orientent simplement l'opérateur vers les entrées qui méritent une relecture.

Au-delà de la correction des champs, chaque fiche extraite passe par un ensemble de 17 règles de cohérence métier automatiques. Ces contrôles s'appuient sur la base cadastrale locale construite au fil des traitements, qui recense aujourd'hui 335 communes de l'Ardèche (696 avec la Drôme) avec l'ensemble de leurs sections connues.

Ces 17 règles se répartissent en quatre groupes de contrôles :
\begin{itemize}
  \item Contrôles des dates (4 règles) : l'algorithme vérifie que l'année se situe dans la plage d'activité du fonds (1950 à aujourd'hui), compare la date avec la période d'exercice du géomètre signataire, valide le format du calendrier (jour, mois, année) et vérifie que la date d'enregistrement reste postérieure à la réalisation du plan.
  \item Contrôles cadastraux et géographiques (5 règles) : le programme croise le code INSEE avec le nom de commune officiel, s'assure que la lettre de section existe bien sur le territoire communal, nettoie les numéros de parcelles pour retirer les lettres parasites issues des tampons, refuse les parcelles numérotées zéro et exige au moins une parcelle rattachée à l'acte.
  \item Contrôles des personnes et intervenants (4 règles) : l'outil vérifie la présence du nom du géomètre dans la liste des géomètres du cabinet, alerte l'opérateur si le champ des anciens propriétaires contient le même nom que celui des nouveaux acquéreurs, supprime les mentions juridiques isolées (comme \enquote{consorts} ou \enquote{veuve}) et contrôle le nom du donneur d'ordre.
  \item Contrôles de dossier et anti-doublon (4 règles) : le script valide la structure de la référence selon le modèle propre à chaque géomètre, vérifie l'absence de doublon dans le registre général, recherche les actes déjà enregistrés sur la même parcelle la même année, et s'assure que le fichier PDF source reste accessible sur le disque.
\end{itemize}
Lorsqu'une fiche enfreint l'un de ces tests, l'interface affiche une pastille d'avertissement orange ou rouge. L'opérateur visualise directement la règle en défaut et corrige la valeur en un clic avant l'exportation finale. 

Certains cas restent hors de portée de l'automatisation : une écriture entièrement illisible, un dossier partiellement scanné ou une commune disparue sans équivalent INSEE nécessitent une décision manuelle. Ces situations sont signalées mais ne peuvent pas être résolues sans intervention de l'opérateur.

Enfin, cette étape de fiabilisation prépare les données pour la dernière phase du projet. Les répertoires propres et les dossiers correctement géolocalisés peuvent à présent être transmis de manière automatique au portail national Géofoncier, sujet présenté dans le chapitre suivant.
