...
Tout l'outil fonctionne directement sur l'ordinateur du cabinet sous Windows. Aucune donnée n'est envoyée sur Internet, ce qui garantit le respect du secret professionnel evoqué plus tôt.
...
Chaque script répond à un rôle précis au sein du flux de travail, facilitant ainsi les évolutions futures. Comme évoqué lors de l'étude des contraintes de sécurité (chapitre~\ref{sec:analyse_metier}), l'ensemble de ces modules s'exécute en local sans dépendance externe vers des API distantes payantes.
...
L'utilisation  de ce modèle pré-entraîné permet d'identifier les boîtes englobantes sans nécessiter une phase d'entraînement. L'algorithme NMS (Non-Maximum Suppression) supprime les encadrements doubles qui se chevauchent sur une même région pour ne conserver que la boîte ayant le meilleur score de confiance. Ces boites peuvent être observées sur la figure \ref{fig:nms_exemple}.
...
\begin{itemize}
  \item Le moteur Tesseract traite les caractères tappés. Il convertit les zones imprimées en chaînes de texte.
...
La Figure~\ref{fig:extraction_ocr_gliner} illustre l'exécution de l'outil sur un documentde type DA.
...
En cas d'échec de réponse du modèle VLM ou si le temps de calcul dépasse un seuil tolérable de 45 secondes, l'arbitrage s'interrompt proprement et attribue la mention par défaut pour que l'opérateur effectue lui-même la vérification dans l'interface web.
...
Même si le système automatise la majorité du travail de lecture, des erreurs peuvent persister, car elles sont existantes sur ces écrits. Il s'agit alors d'une faute, ou la résolution est sujette à interprétation, ou un système d'intelligence artificielle ne peut pas résoudre le problème. Un contrôle humain est alors nécessaire. Le chapitre suivant présente la partie fiabilisation et l'interface graphique permettant à l'utilisateur de contrôler et valider les dossiers avant l'envoi final sur Géofoncier.
