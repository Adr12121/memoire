...
Le chapitre précédent a présenté les outils retenus et les raisons de leur sélection. Ce chapitre décrit comment ils s'assemblent en une chaîne de traitement cohérente, de la réception du document jusqu'au versement des métadonnées sur Géofoncier.
...
Pour l'un des registres du cabinet, un classeur Excel existait déjà avec une partie des informations nécessaire au versement. Un script Python a été écrit pour lire ce classeur et lier directement les pages scannées aux données de ce fichier. Cela évite la saisie manuelle des informations déjà connues.
...
L'ensemble de l'outil fonctionne directement sur l'ordinateur du cabinet sous Windows. Aucune donnée n'est envoyée sur Internet, ce qui garantit le respect du secret professionnel évoqué plus tôt.
...
Du fait de cette problématique, un script Python a été développé pour l'acquisition. Il permet de séparer automatiquement la vue double en deux images distinctes, d'ajuster les marges extérieures et d'attribuer une numérotation propre à chaque page. Cette partie est détaillée dans la sous-section du prétraitement de l'image.


\subsection{Classification et orientation du document}

Le tri permet de savoir quel type de document est à traiter (plan récent fait à l'aide d'un outil de dessin assisté par ordinateur (DAO), plan ancien scanné, registre manuscrit ou document imprimé), et permet d'orienter le traitement en conséquence, vers la bonne partie du programme. Pour gagner du temps, le script \texttt{plan\_classifieur.py} utilise des règles simples qui s'exécutent en quelques secondes.

Le choix repose sur trois critères :

\begin{itemize}
  \item Présence de texte dans le PDF : si le fichier est un PDF issu d'un logiciel de DAO, son texte est directement lisible. La bibliothèque \texttt{PyMuPDF} l'extrait et recherche des mots-clés caractéristiques ("DMPC", "PROCÈS-VERBAL", "LOTISSEMENT", etc.) pour déterminer le type de document sans passer par l'OCR.

  \item Mots-clés détectés par OCR sur la première page : si le PDF est scanné, EasyOCR lit la première page et cherche les mêmes mots-clés, avec une tolérance aux fautes de lecture via du \textit{fuzzy matching} (comparaison approchée qui accepte un écart de quelques caractères entre le mot lu et le mot attendu, pour absorber les erreurs d'OCR comme une lettre manquante ou substituée). Les labels trouvés sans valeur en face (``Commune~:'' seul, sans texte) signalent un formulaire ancien de type DMPC.

  \item Densité des blocs de texte détectés : le script compte le nombre de blocs OCR à haute confiance sur la page. Un document imprimé dense produit beaucoup de blocs fiables. Un croquis ou un registre peu chargé en produit peu, ce qui oriente vers un traitement adapté aux documents anciens ou manuscrits.
\end{itemize}

En cas de doute, le document est envoyé vers la lecture manuscrite. Un modèle capable de lire le manuscrit est aussi capable de lire le texte imprimé, alors qu'un OCR classique échoue sur l'écriture manuelle.

Une fois le document classifié, il est envoyé vers le script \texttt{spatial\allowbreak\_extracteur\allowbreak.py} qui découpe les zones utiles sur la page.
...
Comme le montre ce résultat, YOLOv8 parvient à repérer les informations essentielles du DMPC en traçant un cadre vert pour ce cas autour de chaque élément utile : le nom de la commune, la section cadastrale, le numéro de feuille et l'échelle du plan. Ces zones découpées sont ensuite transmises aux scripts de lecture pour extraire proprement le texte de chaque case.
...
Une fois les zones du document recadrées et nettoyées par le prétraitement, l'étape de lecture s'exécute via le moteur d'extraction (\texttt{main.py} et \texttt{semantic\_ocr\_engine.py}). Le système s'appuie sur le moteur EasyOCR, associé à son détecteur CRAFT, pour délimiter précisément les boîtes de texte et extraire les chaînes de caractères sur les zones découpées.
...
Cette combinaison d'outils permet de traiter l'ensemble des cas sans imposer de réentraînement sur chaque nouveau type de document. Comme nous l'étudierons lors de la présentation de l'interface de validation au chapitre~\ref{sec:fiabilisation}, la note de confiance attribuée à chaque champ détermine la couleur du badge affiché à l'écran, guidant ainsi l'opérateur vers les champs nécessitant une vérification prioritaire.



\subsection{Arbitrage par modèle VLM}

Lorsque l'OCR et GLiNER ne parviennent pas à extraire un champ avec suffisamment de certitude, le projet délègue la lecture à un modèle de vision-langage (VLM) exécuté localement, qui interprète directement l'image de la zone concernée.
...
\begin{quote}
\textit{« Quel est le nom de la commune d'Ardèche écrit dans cette case ? Réponds uniquement par le nom de la commune, sans phrase, et répond par INCERTAIN lorsque tu n'arrives pas à une réponse plausible. »}
\end{quote}
...
\begin{itemize}
  \item Pour les choix pré-imprimés sur un DMPC, afin d'identifier l'option A, B ou C non raturée, information qui peut être utile pour un opérateur.
...
L'utilisation de consignes textuelles ciblées permet d'adapter la sensibilité du modèle VLM selon la nature du champ. Le modèle est configuré pour favoriser la réponse la plus probable plutôt que de varier ses formulations, ce qui l'oblige à retourner des réponses courtes et directes. Ces réponses sont ensuite nettoyées automatiquement par une fonction de filtrage qui supprime la ponctuation inutile et met en majuscules le texte extrait avant sa validation.
...
Une fois les informations extraites et arbitrées, l'ensemble des données est structuré puis soumis à une série de règles de contrôle.
...
Pour s'assurer qu'aucune information absurde ne soit transmise à l'étape suivante, le script nommé 
\texttt{coherence\_controle.py} applique une série de vérifications automatiques :
...
\subsection{Gestion des exceptions et stratégies en cas d'erreur}

Dans une chaîne de traitement automatisée traitant des documents numérisés aux formats variés, des erreurs de calcul ou des échecs de détection peuvent survenir à chaque étape. Pour éviter qu'un fichier défectueux ne bloque l'ensemble du traitement par lot, le script principal intègre une gestion des exceptions associant des stratégies de repli en cas d'erreur.
...
Lorsque l'OCR extrait des chaînes de caractères contenant des octets invalides ou des symboles non imprimables (fréquent sur les tampons), la fonction de nettoyage filtre les caractères hors de la plage UTF-8 standard. Si la chaîne nettoyée est vide, le champ prend la valeur \texttt{INCERTAIN} et le score de confiance est mis à 0, ce qui signale le dossier pour la relecture dans l'interface.
...
De plus, l'ensemble des événements et erreurs survenus lors de l'exécution est consigné dans un fichier de journalisation quotidien (\texttt{traitement.log}). Ce journal d'erreurs indique l'horodatage précis, le nom du fichier concerné, le module informatique à l'origine de l'alerte ainsi que le code de l'erreur. Cette traçabilité permet au responsable du cabinet d'identifier rapidement les documents problématiques et d'ajuster les règles de repli si nécessaire ou de contrôler l'efficacité du système.

Les données extraites et validées par cette chaîne de traitement sont ensuite présentées à l'opérateur dans une interface de contrôle, décrite au chapitre~\ref{sec:fiabilisation}.
