...
Créé en 1807 sous Napoléon I\textsuperscript{er}, le cadastre français avait à l'origine un objectif uniquement fiscal : recenser la totalité des propriétés foncières afin de récolter de façon équitable l'impôt foncier. Aujourd'hui, le Plan Cadastral Informatisé (PCI), géré et mis à jour par la Direction Générale des Finances Publiques (DGFiP), sert de fond de plan de référence pour l'aménagement du territoire.
...
% \begin{figure}[H]
%   \centering
%   % [INDICATEUR FIGURE : Insérer ici un exemple annoté d'un Document Modificatif du Parcellaire Cadastral (DMPC / Cerfa), avec des encadrés de couleur matérialisant les différentes régions structurelles : le cartouche de métadonnées (en haut à droite), le tableau de contenances (au centre) et le plan d'arpentage (en bas).]
%   \rule{0.8\textwidth}{1pt} \\
%   \vspace{0.5em}
%   {\small \textit{Figure à insérer : Exemple d'un DMPC normalisé avec encadrement des zones d'intérêt (cartouche, tableau, plan).}}
%   \caption{Composition d'un Document Modificatif du Parcellaire Cadastral (DMPC).}
%   \label{fig:structure_dmpc}
% \end{figure}

% \begin{figure}[H]
%   \centering
%   % [INDICATEUR FIGURE : Insérer ici une photographie ou un scan d'un PV de bornage narratif historique du cabinet, montrant le texte rédigé en continu, le paragraphe d'en-tête contenant la commune et la date, ainsi que la disposition des signatures à la fin du document.]
%   \rule{0.8\textwidth}{1pt} \\
%   \vspace{0.5em}
%   {\small \textit{Figure à insérer : Exemple d'un PV de bornage narratif historique.}}
%   \caption{Exemple d'un PV de bornage narratif montrant la mise en page sous forme de texte libre.}
%   \label{fig:pv_bornage_narratif}
% \end{figure}
...
Ainsi, la lecture des registres nécessite de traiter ces difficultés de présentation pour extraire correctement les données. Cependant, une fois ces informations récupérées, une autre étape est indispensable : réussir à lier les anciens numéros de parcelles aux références actuelles, qui par supposition ont dû changer au fil des années.


\subsection{La filiation parcellaire}

La filiation parcellaire est importante pour retrouver les parcelles concernées par les archives du cabinet. Lorsqu'un géomètre recherche un dossier aujourd'hui, il dispose souvent uniquement du numéro de la parcelle actuelle. Cette même logique est a appliquer lors de la creation de ces anciens dossiers. Les actes anciens ont été enregistrés sous les numéros parcellaires qui pour la plupart ont subit une modification, un découpage, ou tout autre intervention. L'un des objectif de ce projet est donc de remonter dans le temps pour retrouver les numéros de parcelles d'époques.
