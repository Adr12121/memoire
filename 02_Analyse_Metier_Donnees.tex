...
\begin{figure}[H]
  ...
  \vspace{0.5em}
  \caption{Capture d'écran du portail GéofoncierEXPERT(sans échelle).}
...
Le traitement automatisé vise spécifiquement les actes fonciers ayant une valeur juridique qui sont utiles au versement Géofoncier, à savoir les DMPC, les PV de bornage, ainsi que les registres d'affaires qui les répertorient, car sur ces derniers figurent des informations obligatoires sur les dossiers à verser.

Les DMPC, autrefois appelés DA, sont établis lors des divisions parcellaires pour permettre la mise à jour du plan cadastral par la DGFiP. Rendus obligatoires en France depuis la réforme de la publicité foncière issue du décret n$^\circ$55-22 du 4 janvier 1955 et du décret n$^\circ$55-471 du 30 avril 1955, ces actes visent à garantir la concordance exacte entre les actes notariés publiés et la représentation cadastrale. Ces documents suivent un modèle Cerfa structuré, ce qui peut faciliter le repérage automatique des informations, qui se trouvent au même endroit sur chaque document. Ils comportent un cartouche (précisant la commune, la section et les numéros de parcelles) et un extrait de plan. 
...
Les documents à préremplir (principalement entre 1980 et 2000) ont été créés à la machine à écrire. Les erreurs présent dans le document (taches, ratures, etc.) nécessite un prétraitement pour améliorer la lisibilité.
...
\begin{figure}[H]
  ...
  \caption{Répartition estimée du fonds d'archives du cabinet par noms de géomètres prédécesseurs (Volume total : environ 23~600 dossiers).}
...
Les registres récapitulent l'ensemble des affaires traitées au fil du temps. Dans un registre figure quelques informations clés de chaque dossier : le numéro d'affaire, la date de l'opération, le nom du client, la commune et les références parcellaires. En fonction du fonds, ces informations peuvent ne pas toutes être présentes, rendant l'interprétation de la mission difficile. 

De plus, ces registres ne sont pas tous identiques. Chaque géomètre avait sa propre manière de rédiger. Certains traçaient des colonnes fixes à la règle, d'autres écrivaient en texte libre ou utilisaient des imprimés différents.

Cette diversité entraîne trois difficultés principales pour le projet:

Premièrement, l'absence de grille pour certains registres rend l'alignement des colonnes variable l'une par rapport à l'autre et parfois d'une page à l'autre.
...
Sur les documents anciens rédigés sur du papier acide ou sur des calques, le support jaunit avec le temps et devient cassant. Ce jaunissement réduit le contraste entre le fond du papier et le texte. De plus, sur les calques devenus transparents, l'écriture du verso apparaît au scanner et crée des éléments sources d'erreurs de lecture. Les pochettes d'archives montrent aussi des plis, des déchirures et des traces d'agrafes rouillées. Des notes ajoutées plus tard au crayon ou au stylo par les géomètres croisent parfois le texte d'origine.

Enfin, les tampons de réception du cadastre posés sur les formulaires DMPC recouvrent souvent les numéros de parcelles avec une encre rouge ou bleue. Comme illustré plus loin lors de l'étude des limites (section~\ref{sec:limites_perspectives}), ces éléments visuels perturbent les modèles de lecture automatique.
...
Enfin, la confidentialité et l'exécution locale imposent que l'ensemble des traitements d'analyse d'image et d'extraction de texte s'effectuent directement sur les postes du cabinet, sans envoyer de données sur des serveurs externes distants.
