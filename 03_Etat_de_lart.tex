...
Ce chapitre présente l'état de l'art des technologies disponibles pour l'analyse automatique de documents. Pour chaque méthode, le fonctionnement, les résultats et les limites sont analysés afin de justifier les choix retenus pour la chaîne de traitement.

\subsection{La lecture de texte imprimé (OCR)}
\label{subsec:ocr}

La reconnaissance optique de caractères (OCR - \textit{Optical Character Recognition}) a été conçue pour automatiser la lecture de documents imprimés. L'objectif est de transformer une image de texte en caractères numériques exploitables par un ordinateur.

Le fonctionnement classique repose sur le découpage successif des éléments du document. L'image est d'abord binarisée pour séparer l'encre du papier. Le moteur repère ensuite les lignes de texte, puis découpe chaque ligne en mots et chaque mot en caractères isolés. Enfin, chaque caractère découpé est comparé à des modèles connus pour être identifié. Le moteur Tesseract, par exemple, repère la ligne de base du texte même si la page est penchée ou courbée (comme près de la reliure d'un livre numérisé), il trace une ligne courbe mathématique (une spline quadratique) qui suit l'ondulation des mots. Cela lui permet de lire le texte sans devoir redresser ou déformer l'image au préalable \citep{smith_tesseract_2007}.
...
\begin{itemize}
  \item La transformation géométrique (Trans.) : normaliser la forme de la ligne de texte pour corriger les angles ou les déformations.
  \item L'extraction de caractéristiques (Feat.) : analyser les formes visuelles de l'image à l'aide d'un réseau de convolution (CNN).
  \item La modélisation de séquence (Seq.) : prendre en compte le contexte entre les lettres grâce à un réseau récurrent (BiLSTM).
  \item La prédiction (Pred.) : convertir la séquence visuelle en texte final grâce à une fonction d'alignement (CTC) ou d'attention.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/baek_ocr_pipeline_crop.png}
    \caption{Chaîne de reconnaissance de texte imprimé en quatre étapes. Extrait de \cite{baek_stract_2019}.}
    \label{fig:ocr_pipeline}
\end{figure}

Cette méthode est fiable sur du texte imprimé. En revanche, elle trouve sa limite sur l'écriture manuscrite, car le chevauchement des lettres empêche le modèle de distinguer les caractères correctement et donc de trouver une correspondance avec son entraînement.

Certains moteurs de numérisation s'appuient sur des filtres de binarisation adaptative. La méthode d'Otsu cherche un seuil global optimal en minimisant la variance intra-classe des intensités de gris, ce qui fonctionne bien sur un fond homogène. Néanmoins, la binarisation calcule un seuil local pour chaque pixel en fonction de la moyenne et de l'écart-type de son voisinage. Ce traitement préserve les traits fins des caractères imprimés et présente ainsi un résultat pour l'écriture manuscrite. Le retraitement de l'image peut améliorer significativement la lisibilité de l'écriture et réduire le taux d'erreur de l'OCR.
...
La reconnaissance de texte manuscrit (HTR - \textit{Handwritten Text Recognition}) a été conçue pour lire l'écriture à la main, sur laquelle l'OCR échoue. En écriture manuscrite, les lettres sont attachées, déformées et de tailles variables. Séparer chaque caractère individuellement provoque des erreurs systématiques. Pour résoudre ce problème, les systèmes HTR ne découpent pas les lettres, ils analysent la ligne entière comme un signal visuel continu.

La Figure~\ref{fig:rem_workflow} détaille le fonctionnement général de cette approche en trois étapes :

\begin{itemize}
  \item La segmentation en lignes : le système repère l'emplacement exact de chaque ligne de texte dans la page. Il trace une ligne de base (\textit{baseline}) sous l'écriture et extrait le masque d'image correspondant à la ligne.
  \item L'analyse de la mise en page : le modèle détermine l'ordre logique de lecture des différents blocs ou segments extraits de la page.
  \item La transcription : chaque portion d'image de ligne est convertie directement en une chaîne de caractères.
\end{itemize}
...
La différence entre l'OCR et le HTR, présenté dans la sous-partie précédente, repose sur le type de texte à lire et sur la façon dont le moteur l'analyse :

\begin{itemize}
  \item \textbf{L'OCR} s'applique au texte imprimé. Comme les lettres sont régulières et séparées, le système cherche à isoler chaque caractère ou petit bloc avant de l'identifier.
  \item \textbf{Le HTR} s'applique à l'écriture manuscrite. Comme les lettres varient selon les personnes, le système ne cherche pas à découper les caractères, il lit la ligne entière comme un tracé continu.
\end{itemize}
...
En réalité, beaucoup de documents mélangent du texte imprimé et des ajouts écrits à la main, notamment avec utilisation de documents pré-remplis. Un modèle OCR classique échoue sur les écritures manuscrites, tandis qu'un modèle HTR peut faire des erreurs sur du texte imprimé.
...
\begin{itemize}
  \item \textbf{L'entraînement sur des données mixtes} : certains modèles comme TrOCR \citep{li_trocr_2021}, déjà évoqués, s'appuient sur un pré-entraînement combinant à la fois du texte imprimé et de l'écriture manuscrite. Comme le montre la Figure~\ref{fig:trocr_arch}, le modèle traite l'image de la ligne de façon unifiée et s'adapte automatiquement sans étape de tri préalable.
  \item \textbf{La combinaison de deux outils successifs} : des systèmes comme PaddleOCR \citep{du_ppocr_2020} ou EasyOCR utilisent un premier modèle pour repérer l'emplacement des lignes sur la page (comme DBNet \citep{liao_dbnet_2020}), puis un second modèle chargé uniquement de lire le texte présent dans ces zones.
\end{itemize}
...
Envoyer directement une photographie ou un scan d'un document à un moteur de lecture pose un problème : la page contient de nombreuses zones potentiellement inintéressantes, ou de nombreuses sources d'erreurs. Si le moteur essaie de tout lire sans tri, ce dernier perd du temps et produit du texte parasite qui peut nuire à la qualité des résultats finaux. Il est donc important d'isoler d'abord les zones d'intérêt dans la page avant d'extraire le texte.

Proposé à l'origine par \cite{redmon_yolo_2016}, le modèle YOLO (\textit{You Only Look Once}) a été conçu pour détecter des zones visuelles en un seul passage rapide sur l'image. Depuis sa création, cet algorithme a connu de nombreuses évolutions permettant d'en améliorer la précision et la vitesse \citep{jiang_review_2022}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/yolo_system.pdf}
    \caption{Principe de détection de YOLO : traitement en un seul passage sur une grille spatiale. Extrait de \cite{redmon_yolo_2016}.}
...
L'organisation du réseau de convolution responsable de la détection visuelle y est détaillée. L'image du document traverse d'abord une succession de couches de convolution qui repèrent les détails géométriques simples, pouvant être les lignes, les angles ou encore les contours de cadres. Les couches suivantes combinent ces éléments pour reconnaître des blocs plus complexes, tels que des entêtes, des colonnes ou des cartouches. En parallèle, des opérations de sous-échantillonnage réduisent la taille spatiale pour préserver une vue globale sans alourdir les calculs. Enfin, les dernières couches du réseau convertissent ces caractéristiques visuelles en coordonnées spatiales(position, largeur et hauteur) et attribuent une catégorie à chaque zone identifiée.

Cette architecture permet à YOLO d'analyser rapidement la géométrie d'une page entière et d'isoler les zones d'intérêt.

Contrairement aux approches par modèles de détection classiques reposant sur des boîtes d'ancrage pré-définies, la version v8 de YOLO utilise une architecture sans ancrage. Ce choix simplifie l'apprentissage sur des documents administratifs aux formats non standards, car le réseau prédit directement les coordonnées du centre et les dimensions de chaque zone au lieu d'ajuster des cadres pré-formés. Sur des archives foncières où l'échelle des cartouches et l'emplacement des formulaires varient selon l'époque et l'imprimeur, cette flexibilité peut permettre de détecter les zones utiles avec une meilleure précision qu'un simple découpage à coordonnées fixes.

Comme détaillé ultérieurement lors de l'explication de la chaîne de traitement (chapitre~\ref{sec:architecture}), cette étape de découpage spatial constitue le point d'entrée de toute l'analyse : elle préserve la mémoire de calcul en évitant de soumettre l'intégralité des images aux modèles de reconnaissance de texte en selectionnant uniquement ce qui est nécessaire.
...
Dans les évolutions ultérieures comme LayoutLMv3, les auteurs ont simplifié cette architecture en supprimant la dépendance envers un moteur OCR externe pour l'étape d'analyse visuelle. LayoutLMv3 découpe directement la page scannée en petits patchs d'images (de taille 16 par 16 pixels) traités par un transformateur visuel (Vision Transformer), tout en alignant conjointement les textes et les positions spatiales. Cependant, cette avancée exige un volume d'entraînement considérable sur plusieurs millions de documents annotés. Pour des cabinets de géomètres, le coût de réentraînement complet de telles architectures reste hors de portée.


D'une part, sur des documents imprimés comme les e-mails (99,5~\%), les CV (98,8~\%) ou les dossiers (96,7~\%), le modèle atteint d'excellents résultats. Dans ces cas, les mots reconnus par l'OCR (comme des entêtes d'e-mail ou des sections de CV) fournissent des repères pouvant être exploités.
...
Les outils de NER traditionnels (comme SpaCy ou CamemBERT) fonctionnent avec une liste fermée de catégories définies au moment de leur entraînement, qui peuvent être donc renseigné par l'utilisateur. Pour ajouter de nouvelles catégories spécifiques à un domaine, il faut annoter à la main des fichiers exemples puis réentraîner le modèle, ce qui demande du temps.
...
La Figure~\ref{fig:gliner_architecture} détaille l'organisation de ce réseau à travers plusieurs traitements.
...
Pour compenser les décalages de lignes, les coordonnées verticales des centroïdes des blocs de texte sont regroupées par un algorithme de clustering unidimensionnel (DBSCAN). Ce regroupement permet de rattacher chaque mot à sa ligne d'origine même si le rédacteur a écrit sur plusieurs lignes ou de manière non rectiligne par exemple.

Cette analyse de la mise en page des tableaux est importante pour exploiter les répertoires d'anciens géomètres. Elle permet l'alignement des données avant d'alimenter les modules de detections du projet, pour ne manquer aucun caractère.
...
Pour comparer les performances des modèles d'OCR, de HTR et d'extraction d'entités, la littérature scientifique s'appuie sur plusieurs métriques standardisés.
...
où $S_w$, $D_w$ et $I_w$ représentent respectivement les substitutions, suppressions et insertions de mots entiers, et $N_w$ le nombre total de mots. Le WER est plus sévère que le CER, car une seule lettre de différence rend le mot entier faux.
 ...
 Pour l'extraction d'entités (comme la détection des noms de communes ou des numéros de parcelles par GLiNER), les métriques habituelles sont la précision ($P$), le rappel ($R$) et le score F1 ($F_1$) :
...
Théoriquement, le CER et le WER découlent directement des travaux sur la distance de Levenshtein \citep{levenshtein_binary_1966}, qui compte le nombre minimal de modifications pour passer d'un texte à un autre. De leur côté, la précision, le rappel et le score F1 sont issus du domaine de la recherche d'information et sont universellement employés pour mesurer la qualité des systèmes de classification et d'extraction de données.

Une fois l'ensemble des technologies et de leurs métriques présentées, il convient de les synthétiser afin de comparer leurs caractéristiques et d'orienter les choix d'architecture pour le projet.

\subsection{Synthèse des approches de l'état de l'art}

Chaque technologie étudiée dans ce chapitre répond à une tâche précise de l'analyse de document, qu'il s'agisse d'isoler des zones, de lire du texte ou d'en comprendre le sens. Le Tableau~\ref{tab:comparaison_modeles} regroupe ces différentes méthodes afin de comparer leur fonctionnement, leurs avantages et leurs limites, avec pour objectif de selectionner le plus approprié au cas d'étude.
...
\begin{table}[H]
  ...
  \caption{Synthèse comparative des technologies d'analyse de document, performances déduites des expériences réalisées et de la littérature scientifique.}
  \label{tab:comparaison_modeles}
\end{table}

Pour comparer ces outils, il faut prendre en compte la machine de test. L'ensemble des essais s'appuie sur une configuration standard équipée d'un processeur Intel Core i7, de 32 Go de RAM et d'une carte graphique NVIDIA de 8 Go (VRAM). Ce matériel constitue le cas d'étude, la puissance ne variant que très légèrement entres les postes du cabinet. 

Cette configuration influence directement le choix des modèles du Tableau~\ref{tab:comparaison_modeles}. Les outils légers comme YOLOv8 et GLiNER occupent moins de 2 Go de mémoire graphique et répondent presque instantanément. PyLaia utilise environ 4 Go pour lire une ligne en moins d'une seconde. À l'inverse, les modèles plus lourds comme LayoutLMv3 et LLaVA demandent beaucoup plus de ressources. Avec Ollama, LLaVA prend environ 4,8 secondes par extrait. De plus, son risque principal reste l'hallucination : sur un texte dégradé, le modèle a parfois tendance à inventer une réponse crédible au lieu de signaler qu'il n'arrive pas à lire. Cette hallucination est aussi présente chez Qwen, mais elle est moins fréquente.

Pour compléter ces observations, la Figure~\ref {fig:benchmark_comparatif} compare la précision de chaque méthode (en bleu) et son temps de traitement moyen par zone (en orange). Ces valeurs sont issues des tests présentés dans les articles scientifiques sur des documents administratifs, des registres manuscrits et des documents synthétiques.
...
Pour découper ces lignes sans problématiques de mise en page, la détection avec YOLOv8 \citep{redmon_yolo_2016, jiang_review_2022} permet d'isoler chaque zone de texte avant la lecture.
...
\begin{itemize}
  \item Ultralytics YOLOv8 pour la segmentation spatiale : ce modèle découpe visuellement les zones d'intérêt sur les documents de l'étude sans dépendre du texte.
  \item EasyOCR et PyLaia pour l'extraction textuelle : EasyOCR lit les parties imprimées tandis que PyLaia traite l'écriture manuscrite des documents.
  ...
  \item LLaVA et MiniCPM-V via Ollama en secours local : ces deux modèles VLM s'exécutent uniquement sur les zones incertaines, chacun sélectionné selon le type de document traité.
\end{itemize}

Plusieurs alternatives ont été envisagées et écartées pour des raisons expliquées ci-après, après pour la plupart des tests effectués en interne. Tesseract, moteur OCR classique, produit un taux d'erreur supérieur à 30\,\% dès que le texte est manuscrit, ce qui le rend inutilisable sur les registres anciens. LayoutLM, qui exploite à la fois le texte et la mise en page, voit ses performances chuter (autour de 88\,\% au mieux d'après \citealt{bajrami_comprehensive_2023}) dès que l'OCR sous-jacent commet des erreurs, ce qui arrive fréquemment sur les parties manuscrites. Donut décode directement l'image sans OCR mais ne peut produire que les structures de documents sur lesquels il a été entraîné : il aurait fallu constituer un jeu d'annotation complet de chaque type de formulaire du cabinet, ce qui dépasse les ressources disponibles. Qwen-VL a été écarté en raison d'un temps de calcul trop élevé sur la configuration du cabinet. Phi-3 Vision est quasi exclusivement entraîné sur des données anglophones, ce qui pénalise ses performances sur du texte en français, surtout pour un vocabulaire technique ou cadastral. LLaVA et MiniCPM-V, tous deux exécutés localement via Ollama, ont été retenus comme modèles de secours : LLaVA pour les formulaires imprimés standards et les documents génériques, MiniCPM-V pour les documents dont la mise en page est plus complexe, voire même aléatoire (ex: DMPC, PV d'abornement, croquis). Cette répartition a été établie lors des tests, LLaVA présentant davantage d'hallucinations sur certains types de registres manuscrits, en particulier les DMPC.

Cette architecture garantit le respect du secret professionnel en fonctionnant intégralement en local (chapitre~\ref{sec:analyse_metier}). Elle prépare les données pour la phase de fiabilisation et de validation par l'opérateur (chapitre~\ref{sec:fiabilisation}), avant l'exportation vers le portail Géofoncier (chapitre~\ref{sec:integration}).


Pour le repérage des zones, le modèle YOLOv8 a été choisi. Son rôle est d'isoler les cases des communes, les dates, les tableaux de parcelles ainsi que les tampons permettant d'identifier le géomètre-expert ayant réalisé la mission. Ce choix s'explique par sa rapidité (0,15 seconde par zone), sa faible consommation de mémoire graphique (moins de 2 Go) et son insensibilité au texte. Il permet de découper proprement les images sans dépendre d'un gabarit fixe, et de préparer ainsi les extraits d'images nécessaires au fonctionnement du modèle TrOCR.


Pour la transcription du texte manuscrit, le modèle TrOCR a été sélectionné, dans sa variante francophone \texttt{agomberto/trocr-large-handwritten-fr}. Alors que l'OCR classique comme Tesseract échoue sur l'écriture manuelle, TrOCR lit les registres manuscrits avec précision dès que les lignes lui sont transmises sous forme de découpes isolées par YOLO.

Pour l'extraction des données clés (noms de communes, dates, numéros de parcelles), l'outil GLiNER a été retenu. Contrairement aux modèles comme LayoutLM qui deviennent inefficaces lorsque le texte contient des fautes, GLiNER identifie les entités sans entraînement spécifique et réduit le risque de faux positifs, comme présenté dans ce chapitre.
