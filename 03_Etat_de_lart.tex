...
Avant de concevoir la chaîne de traitement, une étude comparative des technologies des détections existantes est nécessaire. L'objectif est de mettre en place une extraction automatique des données, puis d'effectuer un contrôle humain avant le versement de ces dernières.

Ce chapitre présente six familles de technologies présentant un intérêt pour ce sujet. Pour chaque méthode, le fonctionnement, les résultats et les limites sont analysés pour justifier la solution choisie.
...
Pour bien comprendre la différence entre l'OCR et le HTR, prétenté dans le chapitre précédent, elle repose sur le type de texte à lire et sur la façon dont le moteur l'analyse :
...
\begin{figure}[H]
    ...
    \caption{Architecture globale du modèle GLiNER : comparaison entre les morceaux de texte et les entitées recherchées. Extrait de \cite{zaratiana_gliner_2024}. Ce schéma est également disponible en grand format horizontal en \hyperref[ann:gliner_architecture]{Annexe D}.}
...
\begin{figure}[H]
    ...
    \caption{Les trois tâches du Table Transformer de \citet{smock_table_2022} : détection de la table dans la page (gauche), reconnaissance de sa structure en lignes et colonnes (centre), et analyse fonctionnelle des types de cellules (droite). Source : dépôt officiel Microsoft Table Transformer.}
    \label{fig:table_detection_overview}
\end{figure}

Sur des formulaires anciens numérisés à basse résolution, ces modèles se heurtent aux mêmes problèmes que les méthodes classiques. Les cellules ne sont parfois séparées que par un espace typographique, et les plis du papier peuvent être lus à tort comme des séparateurs. \citet{zhong_image-based_2019} montrent que la position spatiale des blocs de texte, combinée à une analyse des proximités horizontales et verticales, est une alternative robuste quand les séparateurs visuels sont absents.

Dans les registres d'archives historiques où la séparation des colonnes repose sur un alignement visuel sans trait de règle, l'analyse de la densité horizontale des projections de texte permet de détecter les vallées d'espacement (zones blanches verticales régulières). L'algorithme calcule l'histogramme des projections verticales des boîtes de texte sur la largeur de la page et identifie les creux de densité correspondant aux séparations naturelles entre les colonnes de données.

Pour compenser les décalages de ligne sur les registres manuscrits, les coordonnées verticales des centroïdes des blocs de texte sont regroupées par un algorithme de clustering unidimensionnel (DBSCAN). Ce regroupement permet de rattacher chaque mot à sa ligne d'origine même si le rédacteur a écrit de manière oblique ou non rectiligne.

Cette analyse fine de la mise en page des tableaux est indispensable pour exploiter les répertoires d'anciens géomètres, comme présenté au chapitre~\ref{sec:architecture}. Elle garantit l'alignement des données avant d'alimenter les modules de fiabilisation et d'indexation géographique (chapitre~\ref{sec:fiabilisation}).
...
Pour la transcription du texte manuscrit, le modèle HTR PyLaia a été sélectionné. Alors que l'OCR classique comme Tesseract échoue sur l'écriture manuelle, PyLaia obtient un taux d'erreur sur les caractères inférieur à 6 \% dès que les lignes lui sont transmises sous forme de découpes isolées par YOLO.

Pour l'extraction des données clés (noms de communes, dates, numéros de parcelles), l'outil GLiNER a été retenu. Contrairement aux modèles comme LayoutLM qui deviennent inefficaces lorsque le texte contient des fautes, GLiNER permet d'identifier les entités recherchées avec un score F1 de 91,5 \%, sans nécessiter d'entraînement spécifique, et donc de réduire le risque de faux positif ou d'hallucination.

Enfin, pour arbitrer les cas les plus difficiles ou les papiers abîmés, le modèle Vision-Langage LLaVA exécuté en local via le serveur Ollama a été intégré. LLaVA intervient uniquement en dernier recours sur les extraits où le taux de confiance de la chaîne principale est insuffisant. Son exécution locale garantit le secret professionnel, en éliminant tout envoi d'images vers des serveurs extérieurs.
